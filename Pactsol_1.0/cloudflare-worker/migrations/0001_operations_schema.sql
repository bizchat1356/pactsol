...
CREATE UNIQUE INDEX idx_organizations_name_ci
...
CREATE TABLE requirement_counters (
  intake_channel TEXT PRIMARY KEY CHECK (intake_channel IN ('web', 'whatsapp')),
  last_value INTEGER NOT NULL DEFAULT 0
);

INSERT INTO requirement_counters (intake_channel, last_value)
VALUES ('web', 0), ('whatsapp', 0);

CREATE TABLE requirements (
  id TEXT PRIMARY KEY,
  requirement_code TEXT NOT NULL UNIQUE,
  intake_channel TEXT NOT NULL CHECK (intake_channel IN ('web', 'whatsapp')),
  source_reference TEXT,
  organization_id TEXT REFERENCES organizations(id),
  contact_id TEXT REFERENCES contacts(id),
  status TEXT NOT NULL DEFAULT 'received' CHECK (status IN ('received', 'details_pending', 'under_review', 'supplier_discussion', 'quote_ready', 'customer_review', 'confirmed', 'vendor_processing', 'closed')),
  specification TEXT,
  delivery_location TEXT,
  required_by TEXT,
  created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
  UNIQUE(intake_channel, source_reference)
);
